...
// n is at most 19 digits, which means we should use 64 bits to store it.
// Therefore, the set we have to print will have elements at most 3^64, which 
// is much greater than 2^64! So I choose to store all the strings represent 3^k 
// with 0 <= k <= 64. I know it is not the best way...
string three_powers[64] =  {"1",
                            "3",
                            "9",
                            "27",
                            "81",
...
int main() {
    ...
    while (n != 0) {
        if (n == 1) { cout << "{ }" << endl; }
            ...
            string final_str = "{ ";
            n--;
            unsigned long long int bit_test = 1;
            // since n is at most 64 bits long
            for (int i=0; i<64; i++) {
                if (n & bit_test) {
                    final_str += three_powers[i];
                    final_str += ", ";
                }
                bit_test = bit_test << 1ll;
            }
            // last element
            final_str.pop_back();
            final_str.pop_back();
